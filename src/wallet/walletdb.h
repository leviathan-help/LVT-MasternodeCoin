...
public:
    ...
    bool WriteZLVTSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLVTSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLVTSeed_deprecated(uint256& seed);
    bool EraseZLVTSeed();
    bool EraseZLVTSeed_deprecated();

    bool WriteZLVTCount(const uint32_t& nCount);
    bool ReadZLVTCount(uint32_t& nCount);
