...
CzLVTWallet::CzLVTWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zlvt seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLVTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLVTSeed_deprecated()) {
                    LogPrintf("%s: Updated zLVT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zlvt seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zlvt wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzLVTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLVTCount(nCountLastUsed);
    else if (!walletdb.ReadZLVTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLVTWallet::Lock()
{
    seedMaster = 0;
}

void CzLVTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLVTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLVT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLVTWallet::LoadMintPoolFromDB()
{
...
void CzLVTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLVTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLVTWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zlvtTracker->HasPubcoinHash(pMint.first)) {
...
bool CzLVTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLVT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zlvtTracker which also adds to database
    pwalletMain->zlvtTracker->Add(dMint, true);
...
void CzLVTWallet::SeedToZLVT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLVTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLVTWallet::GenerateDeterministicZLVT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLVTWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLVTWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzLVTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
