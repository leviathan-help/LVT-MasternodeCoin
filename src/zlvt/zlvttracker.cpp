...
CzLVTTracker::CzLVTTracker(std::string strWalletFile)
{
...
CzLVTTracker::~CzLVTTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLVTTracker::Init()
{
...
bool CzLVTTracker::Archive(CMintMeta& meta)
{
...
bool CzLVTTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLVTTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLVTTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLVTTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzLVTTracker::GetSpendCache(const uint256& hashStake)
{
...
std::vector<uint256> CzLVTTracker::GetSerialHashes()
{
...
CAmount CzLVTTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLVTTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLVTTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLVTTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLVTTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLVTTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLVTTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLVTTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLVTTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLVTTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLVTTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzLVTWallet* zLVTWallet)
{
    bool iszLVTWalletInitialized = (NULL != zLVTWallet);
    CMintMeta meta;
    ...
    if (! iszLVTWalletInitialized)
        zLVTWallet = new CzLVTWallet(strWalletFile);
    meta.isSeedCorrect = zLVTWallet->CheckSeed(dMint);
    if (! iszLVTWalletInitialized)
        delete zLVTWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzLVTTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLVTTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLVTTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLVTTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLVTTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLVTTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzLVTWallet* zLVTWallet = new CzLVTWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zLVTWallet);
        }
        delete zLVTWallet;
        LogPrint("zero", "%s: added %d dzlvt from DB\n", __func__, listDeterministicDB.size());
    }
