...
bool ValidateAccumulatorCheckpoint(const CBlock& block, CBlockIndex* pindex, AccumulatorMap& mapAccumulators)
{
    ...
    //It is VERY IMPORTANT that when this is being run and height < v2_start, then zLVT need to be disabled at the same time!!
    if (fVerifyingBlocks)
        return true;
...
void AccumulateRange(CoinWitnessData* coinWitness, int nHeightEnd)
{
    ...
    while (pindex && pindex->nHeight <= nHeightEnd) {
        ...
        // 10 blocks were accumulated twice when zLVT v2 was activated
...
bool GenerateAccumulatorWitness(CoinWitnessData* coinWitness, AccumulatorMap& mapAccumulators, CBlockIndex* pindexCheckpoint)
{
    try {
        ...
        if (!coinWitness->nHeightAccEnd) {
            ...
            coinWitness->pWitness = std::unique_ptr<AccumulatorWitness>(new AccumulatorWitness(Params().Zerocoin_Params(), *coinWitness->pAccumulator, *coinWitness->coin));
        }
