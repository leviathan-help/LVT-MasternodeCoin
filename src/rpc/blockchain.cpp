...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zlvtObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zlvtObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zlvtObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zLVTsupply", zlvtObj));
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "  \"zLVTsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zLVT denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zLVT denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zLVT denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zLVT denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zLVT denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zLVT denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zLVT denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zLVT denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zLVT denominations\n"
            "  }\n"
            "}\n"
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in leviathan\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of leviathan addresses\n"
            "     \"leviathanaddress\"   	 	(string) leviathan address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZLVT_FAILED_ACCUMULATOR_INITIALIZATION);
