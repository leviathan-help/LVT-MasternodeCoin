...
#include "config/leviathan-config.h"
#endif
...
void PrepareShutdown()
{
	...
	RenameThread("leviathan-shutoff");
...
void Shutdown()
{
	...
	if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
		lightWorker.StopLightZlvtThread();
	}
#ifdef ENABLE_WALLET
	delete pwalletMain;
	pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
	...
	strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "leviathan.conf"));
...
#ifndef WIN32
	strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "leviathand.pid"));
#endif
	strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
	strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
	strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the LVT and zLVT money supply statistics") + " " + _("on startup"));
	...
	strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 12110, 51474));
	...
	if (GetBoolArg("-help-debug", false))
		strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in LVT/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
			FormatMoney(CWallet::minTxFee.GetFeePerK())));
	strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in LVT/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
	...
	string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, leviathan, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
	if (mode == HMM_BITCOIN_QT)
		debugCategories += ", qt";
	...
	strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in LVT/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
	...
	strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all LEVIATHAN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
	strUsage += HelpMessageGroup(_("Staking options:"));
	strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
	strUsage += HelpMessageOpt("-lvtstake=<n>", strprintf(_("Enable or disable staking functionality for LVT inputs (0-1, default: %u)"), 1));
	strUsage += HelpMessageOpt("-zlvtstake=<n>", strprintf(_("Enable or disable staking functionality for zLVT inputs (0-1, default: %u)"), 1));
	...
	strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:12110"));
	...
	strUsage += HelpMessageOpt("-backupzlvt=<n>", strprintf(_("Enable automatic wallet backups triggered after each zLVT minting (0-1, default: %u)"), 1));
	strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zLVT spends and stakes (0-1, default %u)"), 1));
	...
	strUsage += HelpMessageOpt("-zlvtbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zLVT backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
	...
	strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 12111, 51475));
...
std::string LicenseInfo()
{
	return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		...
		"\n" +
        FormatParagraph(strprintf(_("Copyright (C) %i The LEVIATHAN Developers"), COPYRIGHT_YEAR)) + "\n" +
        "\n" +
...
{
	RenameThread("leviathan-loadblk");
...
/** Sanity checks
 *  Ensure that LEVIATHAN is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
	...
	// Sanity check
	if (!InitSanityCheck())
		return InitError(_("Initialization sanity check failed. LEVIATHAN Coin is shutting down."));
...
#endif
	// Make sure only a single LEVIATHAN process is using the data directory.
	...
	if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
		return InitError(strprintf(_("Cannot obtain a lock on data directory %s. LEVIATHAN Coin is probably already running."), strDataDir));
	...
	if (GetBoolArg("-shrinkdebugfile", !fDebug))
		ShrinkDebugFile();
	LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
	LogPrintf("LEVIATHAN version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
	...
	while (!fLoaded) {
		...
		do {
			try {
				...
				//LEVIATHAN specific: zerocoin and spork DB's
				...
				// LEVIATHAN: load previous sessions sporks if we have them.
				...
				if (GetBoolArg("-reindexmoneysupply", false)) {
					RecalculateZLVTMinted();
					RecalculateZLVTSpent();
					RecalculateLVTSupply(1);
				}
				...
				if (GetBoolArg("-reindexaccumulators", false)) {
					...
					// LEVIATHAN: recalculate Accumulator Checkpoints that failed to database properly
	...
	if (fDisableWallet) {
		...
		if (nLoadWalletRet != DB_LOAD_OK) {
			if (nLoadWalletRet == DB_CORRUPT)
				...
				strErrors << _("Error loading wallet.dat: Wallet requires newer version of LEVIATHAN Coin") << "\n";
			else if (nLoadWalletRet == DB_NEED_REWRITE) {
				strErrors << _("Wallet needed to be rewritten: restart LEVIATHAN Coin to complete") << "\n";
		...
		zwalletMain = new CzLVTWallet(pwalletMain->strWalletFile);
		...
		//Inititalize zLVTWallet
		uiInterface.InitMessage(_("Syncing zLVT wallet..."));

		pwalletMain->InitAutoConvertAddresses();

		bool fEnableZLvtBackups = GetBoolArg("-backupzlvt", true);
		pwalletMain->setZLvtAutoBackups(fEnableZLvtBackups);

		//Load zerocoin mint hashes to memory
		//pwalletMain->zlvtTracker->Init();
	...
	/* Denominations
	   ...
	   For example:
	   1LVT+1000 == (.1LVT+100)*10
	   10LVT+10000 == (1LVT+1000)*10
	*/
	...
	if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
		...
		lightWorker.StartLightZlvtThread(threadGroup);
	}
