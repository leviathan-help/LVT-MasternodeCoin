...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
	if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zLVT stake block is less than allowed start height", __func__);

	if (CZLvtStake* zLVT = dynamic_cast<CZLvtStake*>(stake)) {
		CBlockIndex* pindexFrom = zLVT->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zLVT stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zLVT stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zLVT->GetValue()));
		if (nChecksum200 != zLVT->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zLVT->GetChecksum(), nChecksum200);
	}
	else {
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
	...
	if (txin.IsZerocoinSpend()) {
		...
		stake = std::unique_ptr<CStakeInput>(new CZLvtStake(spend));

		if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
			return error("%s: staked zLVT fails context checks", __func__);
	}
	else {
		...
		CLvtStake* lvtInput = new CLvtStake();
		lvtInput->SetInput(txPrev, txin.prevout.n);
		stake = std::unique_ptr<CStakeInput>(lvtInput);
	}
	...
	if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= Params().Zerocoin_Block_Public_Spend_Enabled() - 1) { //Equivalent for zLVT is checked above in ContextualCheckZerocoinStake()
