...
#include "ui_zlvtcontroldialog.h"
...
std::set<std::string> ZLvtControlDialog::setSelectedMints;
std::set<CMintMeta> ZLvtControlDialog::setMints;

bool CZLvtControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZLvtControlDialog::COLUMN_DENOMINATION || column == ZLvtControlDialog::COLUMN_VERSION || column == ZLvtControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZLvtControlDialog::ZLvtControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZLvtControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZLvtControlDialog::~ZLvtControlDialog()
{
    delete ui;
}

void ZLvtControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZLvtControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZLvtControlWidgetItem* itemDenom(new CZLvtControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZLvtControlWidgetItem *itemMint = new CZLvtControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zlvtTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zlvtTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zLVT.";
            else if (!mint.isSeedCorrect)
                strReason = "The zLVT seed used to mint this zLVT is not the same as currently hold in the wallet";
            else
...
void ZLvtControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZLvtControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZLvt_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZLvtControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZLvtControlDialog::GetSelectedMints()
{
...
void ZLvtControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
