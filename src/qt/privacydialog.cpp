                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zLVT ought to be enough for anybody." - Bill Gates, 2017
    ui->zLVTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzLVTSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zLVTpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzLVT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLVT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zLVT, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zLVT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zLVT in ") +
...
void PrivacyDialog::on_pushButtonSpendzLVT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLVT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zLVT, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zLVT
        sendzLVT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zLVT
    sendzLVT();
}

void PrivacyDialog::on_pushButtonZLvtControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZLvtControlDialog* zLvtControl = new ZLvtControlDialog(this);
    zLvtControl->setModel(walletModel);
    zLvtControl->exec();
}

void PrivacyDialog::setZLvtControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzLvtSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzLVT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid LEVIATHAN Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zLVTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zLVT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " LVT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zLVT</b>";
    ...
    // use mints from zLVT selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZLvtControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZLvtControlDialog::GetSelectedMints();
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zLVT transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zLVT was spent successfully update the addressbook with the label
    ...
    // Clear zlvt selector in case it was used
    ZLvtControlDialog::setSelectedMints.clear();
    ui->labelzLvtSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zLVT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Lvt, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.IsZerocoinMint())
            strStats += tr("zLVT Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zlvtTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zLVT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zLVT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zLVT "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zLVT "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zLVT </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zLVT </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zLVT </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzLVTSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzLVT->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zLVT
        //ui->pushButtonMintzLVT->setEnabled(false);
        //ui->pushButtonMintzLVT->setToolTip(tr("zLVT is currently disabled due to maintenance."));

        // Spend zLVT
        ui->pushButtonSpendzLVT->setEnabled(false);
        ui->pushButtonSpendzLVT->setToolTip(tr("zLVT is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zLVT
        //ui->pushButtonMintzLVT->setEnabled(true);
        //ui->pushButtonMintzLVT->setToolTip(tr("PrivacyDialog", "Enter an amount of LVT to convert to zLVT", 0));

        // Spend zLVT
        ui->pushButtonSpendzLVT->setEnabled(true);
        ui->pushButtonSpendzLVT->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
